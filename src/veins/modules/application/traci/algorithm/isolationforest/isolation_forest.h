...
namespace iforest
{
    ...
    template <typename ValueType, size_t DimensionCount>
    bool isolation_iforest(IsolationForest<ValueType, DimensionCount>& forest, std::vector<Point>& data, uint32_t seed, 
        uint32_t treeCount, uint32_t maxheight)
    {
        std::vector<std::array<float, 4>> forest_data;
        std::vector<double> anomaly_scores;
        for(uint32_t i = 0; i < data.size(); i++)
        {
            std::array<float, 4> point_vec;
            point_vec[0] = data[i].senderPos.length();
            point_vec[1] = data[i].senderCalDensity;
            point_vec[2] = data[i].speed;
            point_vec[3] = data[i].senderFlow;
            forest_data.push_back(point_vec);
        }
        if (!forest.Build(treeCount, seed, forest_data, maxheight))
        {
            return false;
        }

        forest.GetAnomalyScores(forest_data, anomaly_scores);
        for(uint32_t i = 0; i < anomaly_scores.size(); i++)
        {
            data[i].anomaly_score = anomaly_scores[i];
        }
        return  true;
    }
};
